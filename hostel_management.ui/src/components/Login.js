...
const Login = (props) => {
  ...
  const handleLogin = (e) => {
    ...
    if (checkBtn.current.context._errors.length === 0) {
      AuthService.login(username, password, false).then(
        () => {
          props.history.push("/availablerooms");
          window.location.reload();
        },
        (error) => {
          const resMessage =
  ...
  return (
    <div className="col-md-12">
      ...
      <h3 className="center mb-2" style={{"color":"#34495E"}}>Student Login</h3>
        <img
        ...
        <Form onSubmit={handleLogin} ref={form}>
          ...
          <div id="register-link" class="text-center">
              <Link to={"/studentregister"} className="nav-link" style={{"color":"#34495E"}}>
                Register here
              </Link>
          </div>
